...
"[project]/src/app/(user-auth)/login/page.jsx [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function LoginPage() {
    ...
    const submitHandler = async (e_0)=>{
        ...
        try {
            ...
            // SYNC CART
            const cartRes = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$helper$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["client"].post("cart/sync", {
                localCart: JSON.stringify(localCart)
            });
            const cartData = cartRes?.data?.data?.cart || [];
            let finalTotal = 0;
            let originalTotal = 0;
            const items = cartData.filter((item)=>item?.productId).map((item_0)=>{
                const { name, _id, original_price, final_price, discount_percentage, price, thumbnail, stock } = item_0.productId;
                finalTotal += final_price * item_0.qty;
                originalTotal += original_price * item_0.qty;
                return {
                    _id,
                    name,
                    original_price,
...
_s(LoginPage, "34qxETj8lEak+kizo1+e5Yx1qnU=", false, function() {
    return [
...
//# sourceMappingURL=src_27fb7307._.js.map