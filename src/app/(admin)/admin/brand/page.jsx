            ...
            <td className="px-6 py-4">
              <div className="flex flex-wrap gap-2">
                {user.categoryId.map((cat,index) => {
                  return (
                    <span key={index} className="px-3 py-1 text-xs font-medium bg-indigo-100 text-indigo-700 rounded-full">
                      {cat.name}
                    </span>
                  );
                })}
            ...
            <td className="px-6 py-4">
              <div className="flex items-center justify-center gap-3">
                <button className="p-2 rounded-lg hover:bg-blue-50 text-blue-600 hover:text-blue-800 transition">
                  <Link href={`/admin/brand/edit/${user.slug}`}>
                    <Pencil size={18} />
                  </Link>
                </button>

                <DeleteBtn id={user._id} type={"brand"} />
              </div>
            </td>
          </tr>
        ))}
